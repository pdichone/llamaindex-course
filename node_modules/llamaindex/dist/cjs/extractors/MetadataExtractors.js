...
class KeywordExtractor extends _types.BaseExtractor {
   ...
   */ constructor(options){
        ...
        super();
        this.llm = options?.llm ?? new _index.OpenAI();
   ...
   */ async extractKeywordsFromNodes(node) {
        ...
        const completion = await this.llm.complete({
            prompt: (0, _prompts.defaultKeywordExtractorPromptTemplate)({
                contextStr: node.getContent(_schema.MetadataMode.ALL),
...
class TitleExtractor extends _types.BaseExtractor {
   ...
   */ constructor(options){
        ...
        this.nodeTemplate = options?.nodeTemplate ?? (0, _prompts.defaultTitleExtractorPromptTemplate)();
        this.combineTemplate = options?.combineTemplate ?? (0, _prompts.defaultTitleCombinePromptTemplate)();
    }
    /**
   ...
   */ async extract(nodes) {
    ...
    async extractTitles(nodesByDocument) {
        ...
        for (const [key, nodes] of Object.entries(nodesByDocument)){
            ...
            const completion = await this.llm.complete({
                prompt: (0, _prompts.defaultTitleCombinePromptTemplate)({
    ...
    async getTitlesCandidates(nodes) {
        const titleJobs = nodes.map(async (node)=>{
            const completion = await this.llm.complete({
                prompt: (0, _prompts.defaultTitleExtractorPromptTemplate)({
                    contextStr: node.getContent(_schema.MetadataMode.ALL)
                })
            });
...
class QuestionsAnsweredExtractor extends _types.BaseExtractor {
   ...
   */ constructor(options){
        ...
        this.promptTemplate = options?.promptTemplate ?? (0, _prompts.defaultQuestionAnswerPromptTemplate)({
   ...
   */ async extractQuestionsFromNode(node) {
        ...
        const prompt = (0, _prompts.defaultQuestionAnswerPromptTemplate)({
            contextStr,
            numQuestions: this.questions
        });
...
class SummaryExtractor extends _types.BaseExtractor {
   ...
   */ promptTemplate;
    ...
    constructor(options){
        ...
        this.promptTemplate = options?.promptTemplate ?? (0, _prompts.defaultSummaryExtractorPromptTemplate)();
   ...
   */ async generateNodeSummary(node) {
        ...
        const contextStr = node.getContent(this.metadataMode);
        const prompt = (0, _prompts.defaultSummaryExtractorPromptTemplate)({
            contextStr
        });
